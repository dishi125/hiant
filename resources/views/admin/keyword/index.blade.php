...
@section('header-content')
    ...
    <div class="section-header-button">
        <a href="{{ route('admin.keyword.add') }}" class="btn btn-primary">Add New</a>
    </div>
@endsection
                    ...
                    <div class="tab-content" id="myTabContent2">
                        <div class="tab-pane fade show active" id="allData" role="tabpanel" aria-labelledby="comment-data">
                            <div class="table-responsive">
                                <table class="table table-striped" id="Keyword-table">
                                    <thead>
                                        <tr>
                                            <th>Keyword</th>
                                            <th>Order</th>
...
@section('scripts')
    ...
    <script>
        var allTable = "{!! route('admin.keyword.table') !!}";
        var csrfToken = "{{ csrf_token() }}";
        var updateOrder = "{!! route('admin.keyword.update.order') !!}";

        $(function() {
            var allShop = $("#Keyword-table").DataTable({
                responsive: true,
                processing: true,
                serverSide: true,
                deferRender: true,
                ...
                columns: [
                    { data: "keyword", orderable: true },
                    { data: "order", orderable: true },
                    { data: "action", orderable: false },
                ]
            });

            $("#Keyword-table > tbody").sortable({
                items: "tr",
                cursor: "move",
                opacity: 0.6,
                update: function () {
        ...
        function sendOrderToServer() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    $("#Keyword-table").dataTable().api().ajax.reload();
                },
            });
        }
    </script>
